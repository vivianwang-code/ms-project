# decision_evaluator.py - 增強版（包含震盪檢測功能）
...
class DecisionEvaluator:
    ...
    # def export_detailed_report(self, filepath=None):
    #     """導出詳細報告"""
    #     if filepath is None:
    #         filepath = f"decision_evaluator_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        
    #     report_lines = []
    #     report_lines.append("=" * 80)
    #     report_lines.append("增強版DecisionEvaluator詳細報告")
    #     report_lines.append("=" * 80)
    #     report_lines.append(f"報告生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    #     report_lines.append("")
        
    #     # 基本統計
    #     report_lines.append("基本統計信息:")
    #     report_lines.append(f"  - 總決策記錄數: {len(self.decision_history)}")
    #     report_lines.append(f"  - 總評估次數: {len(self.evaluation_results)}")
    #     report_lines.append(f"  - 震盪檢測: {'啟用' if self.oscillation_detection_enabled else '禁用'}")
    #     report_lines.append(f"  - 自動關機建議: {'啟用' if self.auto_shutdown_enabled else '禁用'}")
    #     report_lines.append("")
        
    #     # 震盪檢測摘要
    #     if self.oscillation_detection_enabled:
    #         osc_summary = self.get_oscillation_summary()
    #         report_lines.append("震盪檢測摘要:")
    #         report_lines.append(f"  - 總震盪事件: {osc_summary['total_oscillation_events']}")
    #         report_lines.append(f"  - 總關機事件: {osc_summary['total_shutdown_events']}")
    #         report_lines.append(f"  - 平均震盪強度: {osc_summary['average_oscillation_intensity']:.3f}")
    #         report_lines.append(f"  - 總震盪時間: {osc_summary['total_oscillation_minutes']:.1f} 分鐘")
    #         report_lines.append(f"  - 當前是否震盪: {'是' if osc_summary['current_oscillating'] else '否'}")
    #         report_lines.append("")
        
    #     # 評估摘要
    #     eval_summary = self.get_evaluation_summary()
    #     if 'message' not in eval_summary:
    #         report_lines.append("評估摘要:")
    #         report_lines.append(f"  - 最新綜合評分: {eval_summary['latest_overall_score']:.3f}")
    #         report_lines.append(f"  - 歷史平均評分: {eval_summary['historical_average_score']:.3f}")
    #         report_lines.append(f"  - 評分趨勢: {eval_summary['score_trend']}")
    #         report_lines.append(f"  - 最新表現等級: {eval_summary['latest_performance']}")
    #         report_lines.append(f"  - 高優先級建議數: {eval_summary['high_priority_recommendations']}")
    #         report_lines.append(f"  - 系統穩定性: {'穩定' if eval_summary['system_stability'] else '不穩定'}")
    #         report_lines.append("")
        
    #     # 最近的評估結果詳情
    #     if self.evaluation_results:
    #         report_lines.append("最近3次評估結果詳情:")
    #         for i, result in enumerate(self.evaluation_results[-3:], 1):
    #             report_lines.append(f"  評估 {i} ({result['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}):")
    #             report_lines.append(f"    - 綜合評分: {result['overall_score']['overall_score']:.3f}")
    #             report_lines.append(f"    - 穩定性: {'穩定' if result['volatility_metrics']['is_stable'] else '不穩定'}")
    #             report_lines.append(f"    - 一致性分數: {result['consistency_metrics']['consistency_score']:.3f}")
    #             report_lines.append(f"    - 總錯誤率: {result['error_metrics']['total_error_rate']:.3f}")
    #             report_lines.append(f"    - 優先級: {result['recommendation']['priority']}")
    #             report_lines.append("")
        
    #     # 建議匯總
    #     if self.evaluation_results:
    #         all_recommendations = []
    #         for result in self.evaluation_results[-5:]:  # 最近5次評估
    #             all_recommendations.extend(result['recommendation']['recommendations'])
            
    #         unique_recommendations = list(set(all_recommendations))
    #         if unique_recommendations:
    #             report_lines.append("累積改進建議:")
